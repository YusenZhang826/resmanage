...
public class ExcelUtil<T> {
    ...
    public List<T> importExcel(InputStream is, String sheetname) throws Exception {
        return importExcel(is, 0, sheetname);
    }
     ...
     * @return 转换后集合
     */
    public List<T> importExcel(InputStream is, int titleNum, String sheetname) throws Exception {
        return importExcel(sheetname, is, titleNum);
    }
    ...
    public static void checkExcelFile(MultipartFile file) {
        ...
        if (!fileName.matches("^.+\\.(?i)(xls)$") && !fileName.matches("^.+\\.(?i)(xlsx)$")) {
            // 文件格式不正确
            errorInfo = "Excel文件格式不正确";
        }

        long size = file.getSize();
        if (StringUtils.isEmpty(fileName) || size == 0) {
            // 文件不能为空
            errorInfo = "Excel文件内容为空";
        }
        if (errorInfo != null) {
